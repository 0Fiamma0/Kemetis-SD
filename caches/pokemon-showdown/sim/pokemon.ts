...
export class Pokemon {
	...
	*/
	...
	heal(d: number, source: Pokemon | null = null, effect: Effect | null = null) {
		if (!this.hp && effect?.id !== 'secondchance') return false;
		d = this.battle.trunc(d);
		if (isNaN(d) && effect?.id !== 'secondchance') return false;
		if (d <= 0 && effect?.id !== 'secondchance') return false;
	...
	sethp(d: number) {
		if (!this.hp && effect?.id !== 'secondchance') return 0;
		d = this.battle.trunc(d);
		if (isNaN(d) && effect?.id !== 'secondchance') return;
		if (d < 1) d = 1;
		d = d - this.hp;
		this.hp += d;
		if (this.hp > this.maxhp) {
