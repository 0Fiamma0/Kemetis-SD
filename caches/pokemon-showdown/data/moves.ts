...
export const Moves: {[moveid: string]: MoveData} = {
	...
	mistyterrain: {
		...
		condition: {
			...
			onBasePowerPriority: 6,
			onBasePower(basePower, attacker, defender, move) {
				if (move.id === 'dragonmist') {
					this.debug('misty terrain boost');
					return this.chainModify(1.5);
				}
				else if (move.type === 'Dragon' && defender.isGrounded() && !defender.isSemiInvulnerable()) {
	...
	flowerblessing: { //Fiorbenedizione
		num: -3,
		accuracy: true,
		basePower: 0,
		category: "Status",
	...
	spectrallullaby: { //Ninnananna Spettrale
		num: -4,
		accuracy: 80,
		basePower: 0,
		category: "Status",
	...
	electrodrain: { //Elettrassorbimento
		num: -8,
		accuracy: 100,
		basePower: 80,
		category: "Special",
	...
	stealthattack: { //Furtiattacco
		...
		onTry(source) {
			if (source.activeMoveActions > 1) {
				this.hint("Stealth Attack only works on your first turn out.");
				return false;
			}
		},
		secondary: {
	...
	changeofheart: { //Mutacuore
		...
		heal: [1, 2],
		onHit(target, pokemon, move) {
			if (pokemon.baseSpecies.baseSpecies === 'Emutep' && !pokemon.transformed) {
				move.willChangeForme = true;
			}
		},
		onAfterMoveSecondarySelf(pokemon, target, move) {
			if (move.willChangeForme) {
				const newForme = pokemon.species.id === 'emutepsorrow' ? 'Emutep' : 'Emutep-Sorrow';
				pokemon.formeChange(newForme, this.effect, false, '[msg]');
				const newAbility = (newForme === 'Emutep') ? 'soulkindle' : 'soulshroud';
				if (pokemon.setAbility(newAbility)) {
