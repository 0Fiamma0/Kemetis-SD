...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	soulkindle: {
		...
		onAllyModifyAtk(atk, target, source, move) {
			return this.chainModify([4506, 4096]);
		},
		onAllyModifySpA(spa, target, source, move) {
			return this.chainModify([4506, 4096]);
		},
		flags: {},
		name: "Soulkindle",
	...
	soulshroud: {
		...
		onAllyModifyDef(def, target, source, move) {
			return this.chainModify([4506, 4096]);
		},
		onAllyModifySpD(spd, target, source, move) {
			return this.chainModify([4506, 4096]);
		},
		flags: {},
		name: "Soulshroud",
	...
	peaceofmind: {
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Ghost') {
				if (!this.heal(target.baseMaxhp / 4)) {
					this.add('-immune', target, '[from] ability: Peace Of Mind');
				}
				return null;
			}
		},
